          ...
          <xsl:choose>
            <xsl:when test="@scheme">
              ...
              <xsl:value-of select="@scheme"/><xsl:text>',</xsl:text>
                  ...
                  <xsl:when test="$is_first_test and not(preceding-sibling::request) and /imprimatur/@scheme">
                    <xsl:text>&#10;        'scheme': '</xsl:text>
                    <xsl:value-of select="/imprimatur/@scheme"/>
                    <xsl:text>',</xsl:text>
                  </xsl:when>
                ...
                <xsl:choose>
                  ...
                  <xsl:when test="not(preceding-sibling::request) and not(preceding-sibling::request) and /imprimatur/@port">
                    <xsl:text>&#10;        'port': '</xsl:text>
                    <xsl:value-of select="/imprimatur/@port"/>
                    <xsl:text>',</xsl:text>
                  </xsl:when>
      ...
      <xsl:choose>
        <xsl:when test="name() = 'test'">
          <xsl:variable name="name" select="@name" /> 
            <xsl:variable name="is_first_test">
              <xsl:choose>
                <xsl:when test="not(preceding-sibling::test)">
          ...
          <xsl:for-each select="request | comment()">
            <xsl:choose>
              <xsl:when test="name() = 'request'">
                <xsl:variable name="is_first_request">
                  <xsl:choose>
                    <xsl:when test="not(preceding-sibling::request)">
                      <xsl:value-of select="true()"/>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of select="false()"/>
                ...
                <xsl:text>k</xsl:text><xsl:value-of select="$is_first_test"/><xsl:text>k</xsl:text>
                <xsl:text>y</xsl:text><xsl:value-of select="$is_first_request"/><xsl:text>y</xsl:text>
                <xsl:text>p</xsl:text><xsl:value-of select="boolean(boolean($is_first_request) and boolean($is_first_test))"/><xsl:text>p</xsl:text>

                <xsl:variable name="is_first_ever_request">
                  <xsl:choose>
                    <xsl:when test="$is_first_test and $is_first_request">
                      <xsl:value-of select="true()"/>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of select="false()"/>
                ...
                <xsl:if test="$is_first_request">
                    <xsl:text>first request is tur</xsl:text>
                  <xsl:if test="$is_first_test">
                    <xsl:text>truuuu</xsl:text>
                  </xsl:if>
                </xsl:if>

                <xsl:text>&#10;    {</xsl:text>

                <xsl:if test="not(preceding-sibling::request)">
                  <xsl:text>&#10;        'name': "</xsl:text>
                  <xsl:value-of select="$name"/>
                  <xsl:text>",</xsl:text>
                </xsl:if>
