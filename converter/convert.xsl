      ...
      <xsl:for-each select="request">
        ...
        <xsl:choose>
          ...
          <xsl:when test="$is_first and /imprimatur/@scheme">
            <xsl:text>&#10;        'scheme': '</xsl:text>
            <xsl:value-of select="/imprimatur/@scheme"/><xsl:text>',</xsl:text>
          </xsl:when>
        </xsl:choose>
        ...
        <xsl:choose>
          ...
          <xsl:when test="$is_first and /imprimatur/@port">
            <xsl:text>&#10;        'port': '</xsl:text>
            <xsl:value-of select="/imprimatur/@port"/><xsl:text>',</xsl:text>
          </xsl:when>
        </xsl:choose>
        ...
        <xsl:if test="credentials">
          <xsl:text>&#10;        'auth': ('</xsl:text>
          <xsl:value-of select="credentials/@username"/>
          <xsl:text>', '</xsl:text>
        ...
        <xsl:if test="control/@type = 'file'">
          <xsl:text>&#10;        'files': {'</xsl:text>
          <xsl:value-of select="control/@name"/><xsl:text>': '</xsl:text>
          <xsl:value-of select="control/@value"/><xsl:text>'},</xsl:text>
        </xsl:if>

        <xsl:if test="control and not(control/@type)">
          <xsl:text>&#10;        'data': {</xsl:text>
        ...
        <xsl:if test="response-code">
          ...
          <xsl:value-of select="response-code/@value"/><xsl:text>,</xsl:text>
        </xsl:if>
